...
export default function Suggest(){
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        const response = await axios.post('/api/suggestDish', {dishname: dishName, info: info})
    ...
    return(
        <>
            ...
            <main className="h-auto w-full min-h-screen flex bg-diner">
                <form action="/api/suggestDish" method="post" onSubmit={handleSubmit} className="h-96 w-2/5 m-auto bg-white flex flex-col rounded-md">
                    <h4 className="italic font-semibold text-3xl w-full text-center">Suggest A New Dish</h4>
                    ...
                    <textarea placeholder="Enter any addtional information about dish" value={info} onChange={(e) => setInfo(e.target.value)}  className="w-4/5 h-28 mx-auto border border-y-2 border-x-2 bg-gray-300"/>
                    <button type="submit" className="text-white bg-slate-800 w-3/5 h-12 mx-auto mt-5 hover:scale-105 duration-700">Make Suggestion</button>
                </form>
                <aside className="flex flex-col space-y-2 fixed z-30 right-1 bottom-10">
                    <a href="/">
                    <div className="group w-16 h-16 bg-white rounded-full flex justify-center items-center hover:bg-black hover:text-white cursor-pointer shadow-2xl">
